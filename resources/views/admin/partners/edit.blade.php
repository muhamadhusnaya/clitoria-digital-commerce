            ...
            <div class="bg-surface-container-lowest overflow-hidden shadow-[0_10px_30px_-10px_rgba(31,35,64,0.04)] sm:rounded-2xl border border-outline-variant/30 p-8">
                <form action="{ route('admin.partners.update') }" method="POST">
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-outline-variant/30">
                        ...
                        <a href="{ route('admin.partners.index') }" class="px-6 py-2.5 bg-surface-container-high text-on-surface-variant rounded-full text-sm font-medium hover:bg-surface-container transition-all border border-outline-variant/50">
                            Cancel
                        </a>
                    </div>
                </form>
