<x-app-layout>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="title" id="title" value="{{ old('title', $gallery->title) }}" required class="w-full bg-[#F4F2FF] dark:bg-[#3E3E3A] border border-[#797584] dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#432B9F] focus:border-[#432B9F] transition-all outline-none">
                                </div>

                                <div>
                                    <label for="description" class="block text-sm font-medium text-[#151936] dark:text-gray-200 mb-1">Description</label>
                                    <textarea name="description" id="description" rows="4" class="w-full bg-[#F4F2FF] dark:bg-[#3E3E3A] border border-[#797584] dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#432B9F] focus:border-[#432B9F] transition-all outline-none">{{ old('description', $gallery->description) }}</textarea>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" required class="w-full bg-[#F4F2FF] dark:bg-[#3E3E3A] border border-[#797584] dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#432B9F] focus:border-[#432B9F] transition-all outline-none">
                                        <option value="active" {{ old('status', $gallery->status ? 'active' : 'inactive') == 'active' ? 'selected' : '' }}>Active (Visible on public gallery)</option>
                                        <option value="inactive" {{ old('status', $gallery->status ? 'active' : 'inactive') == 'inactive' ? 'selected' : '' }}>Inactive (Hidden)</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mb-3">Leave blank if you don't want to change the current image.</p>
                                ...
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#797584] dark:border-gray-600 border-dashed rounded-xl bg-[#F4F2FF] dark:bg-[#3E3E3A] relative group hover:border-[#432B9F] transition-colors" id="drop-zone">
                                    <div class="space-y-2 text-center relative z-10 hidden" id="upload-prompt">
                                    ...
                                    <div id="image-preview-container" class="absolute inset-0 rounded-xl overflow-hidden bg-black/10 flex items-center justify-center p-2">
                                        <img id="image-preview" src="{{ Str::startsWith($gallery->image, 'http') ? $gallery->image : asset('storage/' . $gallery->image) }}" alt="Preview" class="max-h-full max-w-full object-contain rounded-lg shadow-sm">
                                        ...
                                        <div class="absolute bottom-2 left-2 bg-black/70 text-white text-xs px-2 py-1 rounded">Current Image</div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Store original source to revert if needed
            ...
            // Handle remove image (revert to upload prompt)
            ...
            function showPreview(file) {
                ...
                reader.onload = function(e) {
                    ...
                    // Update label from "Current Image" to "New Image"
                    const label = previewContainer.querySelector('.absolute.bottom-2');
                    if(label) label.textContent = 'New Selected Image';
