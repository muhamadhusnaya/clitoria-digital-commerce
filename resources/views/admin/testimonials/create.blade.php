<x-app-layout>
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-10 flex flex-col items-start gap-4">
            ...
            <h2 class="font-headline-md text-[32px] font-bold text-on-surface">Buat Testimoni Baru</h2>
        </div>
                ...
                <section class="bg-surface-container-lowest rounded-xl p-8 shadow-[0_10px_30px_-5px_rgba(31,35,64,0.04)] border border-outline-variant/30">
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <input name="name" value="{{ old('name') }}" required class="w-full h-[56px] rounded-xl border-outline-variant bg-surface-container-low px-4 text-on-surface focus:ring-2 focus:ring-primary transition-all placeholder:text-outline text-[16px]" placeholder="Sarah Jenkins" type="text">
                        </div>
                        
                        <div class="col-span-2 md:col-span-1">
                            <label class="block font-medium text-[14px] text-on-surface-variant mb-2">Pekerjaan/Peran</label>
                            <input name="role" value="{{ old('role') }}" class="w-full h-[56px] rounded-xl border-outline-variant bg-surface-container-low px-4 text-on-surface focus:ring-2 focus:ring-primary transition-all placeholder:text-outline text-[16px]" placeholder="Wellness Coach" type="text">
                        </div>
                        
                        <div class="col-span-2">
                            <label class="block font-medium text-[14px] text-on-surface-variant mb-2">Perusahaan (Opsional)</label>
                            <input name="company" value="{{ old('company') }}" class="w-full h-[56px] rounded-xl border-outline-variant bg-surface-container-low px-4 text-on-surface focus:ring-2 focus:ring-primary transition-all placeholder:text-outline text-[16px]" placeholder="Mis. Google, Meta" type="text">
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="content" required class="w-full rounded-xl border-outline-variant bg-surface-container-low p-4 text-on-surface focus:ring-2 focus:ring-primary transition-all placeholder:text-outline resize-none text-[16px]" placeholder="Tulis ulasan pelanggan di sini..." rows="6">{{ old('content') }}</textarea>
                        </div>
                        ...
                        <div class="col-span-2 pt-4">
                            <div class="flex items-center justify-between p-4 bg-primary-fixed/30 rounded-xl border border-primary-fixed">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="status" value="active" class="sr-only peer" {{ old('status', 'active') == 'active' ? 'checked' : '' }}>
                ...
                <section class="bg-surface-container-lowest rounded-xl p-8 shadow-[0_10px_30px_-5px_rgba(31,35,64,0.04)] border border-outline-variant/30">
                    ...
                    <div class="mt-10 p-6 rounded-xl bg-surface-bright border border-outline-variant/30 flex items-start space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-[12px] text-on-surface-variant leading-relaxed mt-1">Untuk hasil terbaik, gunakan foto potret berkualitas tinggi dengan latar belakang bersih. Rasio ideal 1:1.</p>
                        </div>
                    </div>
                </section>
            </div>
