<x-app-layout>
    ...
    <div class="mb-10 flex flex-col items-start gap-4">
        ...
        <h2 class="font-display-lg text-[48px] leading-[56px] font-bold text-on-background">Edit Harga/Paket</h2>
        <p class="text-on-surface-variant font-body-md text-[16px]">Perbarui detail tingkatan harga dan jenis paket untuk produk botani Anda.</p>
    </div>
    ...
    <form action="{{ route('admin.product-prices.update', $product_price->id) }}" method="POST">
        @csrf
        @method('PUT')
        <!-- Content Grid -->
                ...
                <div class="bg-surface-container-lowest rounded-xl p-8 shadow-[0px_10px_30px_rgba(31,35,64,0.04)] border border-outline-variant/30">
                    ...
                    <div class="space-y-6">
                            ...
                            <select name="product_id" id="product_id" required class="h-[56px] px-6 rounded-xl border-outline-variant bg-surface-container-low text-on-surface text-[16px] transition-all appearance-none cursor-pointer focus:border-primary focus:ring focus:ring-primary/20">
                                ...
                                @if(isset($products))
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ old('product_id', $product_price->product_id) == $product->id ? 'selected' : '' }} data-name="{{ $product->name }}">
                                            {{ $product->name }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input name="package_name" id="package_name-input" value="{{ old('package_name', $product_price->package_name) }}" required class="h-[56px] px-6 rounded-xl border-outline-variant bg-surface-container-low text-on-surface text-[16px] transition-all focus:border-primary focus:ring focus:ring-primary/20" placeholder="Mis. Eco Sachet, Premium Tin Box" type="text"/>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="flex flex-col gap-2">
                                ...
                                <select name="type" id="type" required class="h-[56px] px-6 rounded-xl border-outline-variant bg-surface-container-low text-on-surface text-[16px] transition-all appearance-none cursor-pointer focus:border-primary focus:ring focus:ring-primary/20">
                                    <option value="single" {{ old('type', $product_price->type) == 'single' ? 'selected' : '' }}>Single Item</option>
                                    <option value="bundle" {{ old('type', $product_price->type) == 'bundle' ? 'selected' : '' }}>Bundle Package</option>
                                </select>
                            </div>
                            <div class="flex flex-col gap-2">
