<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{ route('admin.galleries.create') }" class="px-4 py-2 bg-primary text-white rounded-full text-sm font-medium hover:bg-primary-container transition-all shadow-[0_4px_14px_0_rgba(67,43,159,0.39)]">
                Add New Gallery
            </a>
        </div>
    </x-slot>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-surface-container-low border-b border-outline-variant/50">
                                    ...
                                    <th class="px-6 py-4 text-xs tracking-wider uppercase font-semibold text-on-surface-variant">Title</th>
                                    <th class="px-6 py-4 text-xs tracking-wider uppercase font-semibold text-on-surface-variant">Image</th>
                                    <th class="px-6 py-4 text-xs tracking-wider uppercase font-semibold text-on-surface-variant">Status</th>
                            ...
                            <tbody>
                                @forelse ($galleries as $gallery)
                                    <tr class="border-b border-outline-variant/30 hover:bg-surface-container-low/50 transition-all duration-200">
                                        <td class="px-6 py-4 font-medium text-primary">#{ $gallery->id }</td>
                                                                                <td class="px-6 py-4 text-sm text-on-surface-variant">Data...</td>
                                        <td class="px-6 py-4 text-sm text-on-surface-variant">Data...</td>
                                        <td class="px-6 py-4 text-sm text-on-surface-variant">Data...</td>

                                        <td class="px-6 py-4 text-right">
                                            <a href="{ route('admin.galleries.edit', $gallery->id) }" class="inline-flex items-center justify-center w-8 h-8 mr-2 bg-primary-fixed text-on-primary-fixed rounded-full hover:bg-primary-fixed-dim transition-colors" title="Edit">
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-on-surface-variant">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p>No data found.</p>
                                                <a href="{ route('admin.galleries.create') }" class="mt-2 text-primary hover:underline font-medium">Create one now</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
