<x-app-layout>
    <div class="max-w-[1000px] mx-auto py-10">
        <div class="flex flex-col gap-6 mb-12">
            ...
            <h2 class="text-4xl font-bold text-[#151936] tracking-tight">Edit Manfaat</h2>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div class="md:col-span-7 flex flex-col gap-8">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input name="title" value="{{ old('title', $benefit->title) }}" required class="h-[56px] px-6 rounded-xl border-0 bg-[#f4f2ff] focus:ring-2 focus:ring-[#432b9f] focus:bg-white shadow-sm transition-all duration-300 text-[16px] placeholder:text-[#797584]" id="title" placeholder="cth. Kekuatan Antioksidan Alami" type="text"/>
                    </div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="status" id="status" class="h-[56px] px-6 rounded-xl border-0 bg-[#f4f2ff] focus:ring-2 focus:ring-[#432b9f] focus:bg-white shadow-sm transition-all duration-300 text-[16px]">
                            <option value="1" {{ old('status', $benefit->status) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status', $benefit->status) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                    </div>
                ...
                <div class="md:col-span-5 flex flex-col gap-2">
                    ...
                    <div class="mb-4">
                        <select name="icon_type" id="icon_type" class="w-full h-[56px] px-6 rounded-xl border-0 bg-[#f4f2ff] focus:ring-2 focus:ring-[#432b9f] focus:bg-white shadow-sm transition-all duration-300 text-[16px]">
                            <option value="material" {{ old('icon_type', $benefit->icon_type) === 'material' ? 'selected' : '' }}>Ikon Material</option>
                            <option value="image" {{ old('icon_type', $benefit->icon_type) === 'image' ? 'selected' : '' }}>Unggah Gambar</option>
                        </select>
                    </div>

                    <!-- Material Icon Text -->
                    <div id="material-input-group" class="mb-4">
                        <input name="icon" value="{{ old('icon', $benefit->icon_type === 'material' ? $benefit->icon : '') }}" class="w-full h-[56px] px-6 rounded-xl border-0 bg-[#f4f2ff] focus:ring-2 focus:ring-[#432b9f] focus:bg-white shadow-sm transition-all duration-300 text-[16px]" placeholder="cth. eco, health_and_safety"/>
                    </div>
                    ...
                    <div id="image-upload-group" class="hidden relative group cursor-pointer h-[200px] rounded-xl border-2 border-dashed border-[#c9c4d5] bg-[#f4f2ff] hover:bg-[#e6deff]/50 hover:border-[#432b9f] transition-all duration-300 flex flex-col items-center justify-center text-center p-8 overflow-hidden">
                        <div class="flex flex-col items-center gap-4 transition-transform group-hover:scale-105 duration-300">
                            @if($benefit->icon_type === 'image' && $benefit->icon)
                                <img src="{{ Storage::url($benefit->icon) }}" class="w-16 h-16 object-cover rounded-xl shadow-sm mb-2" />
                            @else
                            ...
                            @endif
                            <div>
                                <p class="text-[14px] font-bold text-[#151936]">Klik untuk mengubah atau seret dan lepas</p>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Trigger change on load to set correct initial state
            iconTypeSelect.dispatchEvent(new Event('change'));
        });
    </script>
</x-app-layout>
