<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-6">
            ...
            <h2 class="text-3xl font-semibold text-[#151936]">Edit Galeri</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <form class="grid grid-cols-12 gap-6 items-start" id="gallery-form"
              action="{{ route('admin.galleries.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <section class="col-span-12 lg:col-span-5 space-y-8">
                <div class="p-8 bg-white rounded-lg shadow-sm border border-[#c9c4d5]/30 space-y-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="title" name="title" value="{{ old('title', $gallery->title) }}" required
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="6"
                                  ...
                                  placeholder="Ceritakan kisah di balik koleksi galeri ini...">{{ old('description', $gallery->description) }}</textarea>
                    </div>
                    <!-- Status Dropdown -->
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select id="is_published" name="is_published"
                                ...
                                <option value="0" {{ old('is_published', $gallery->is_published) == '0' ? 'selected' : '' }}>Draf</option>
                                <option value="1" {{ old('is_published', $gallery->is_published) == '1' ? 'selected' : '' }}>Dipublikasi</option>
                            </select>
            ...
            <section class="col-span-12 lg:col-span-7">
                <div class="p-8 bg-white rounded-lg shadow-sm border border-[#c9c4d5]/30">
                    ...
                    <div class="border-2 border-dashed border-[#432b9f]/30 bg-[#fbf8ff] min-h-[420px] rounded-lg flex flex-col items-center justify-center p-12 group hover:bg-[#5b46b8]/5 transition-all cursor-pointer" id="dropzone">
                        ...
                        <h4 class="text-[24px] font-semibold text-[#151936] mb-2">Klik untuk mengubah gambar</h4>
                        <p class="text-[16px] text-[#484553] text-center max-w-sm">
                            Fotografi botani resolusi tinggi. Mendukung JPG, PNG, WEBP (Maks 2MB).
                        </p>
                        <!-- Preview -->
                        <div class="mt-10 w-full max-w-md {{ $gallery->image ? '' : 'hidden' }}" id="preview-container">
                            <div class="aspect-[4/3] bg-[#e6e6ff] rounded-lg flex items-center justify-center overflow-hidden">
                                <img id="preview-image" src="{{ $gallery->image ? Storage::url($gallery->image) : '#' }}" alt="Pratinjau" class="w-full h-full object-cover" />
                            </div>
                        </div>
                    </div>
                    <!-- Preview Hint -->
