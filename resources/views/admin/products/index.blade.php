<x-app-layout>
    ...
    <div class="py-1">
        ...
        $drafts = $products->count() - $published;
        @endphp

        <!-- Bento Stats Grid -->
        <div class="grid gap-6 mb-12" style="grid-template-columns: repeat(5, minmax(0, 1fr));">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-[0_10px_30px_-5px_rgba(31,35,64,0.04)] flex flex-col gap-1 border border-[#c9c4d5]/30">
                <span class="text-[#797584] uppercase text-[11px] tracking-widest font-semibold">Stok Menipis</span>
                <span class="text-3xl font-bold text-[#e8864a]">0</span>
            </div>
        </div>
    </div>
    ...
    <section class="bg-[#ffffff] rounded-xl shadow-[0_10px_30px_rgba(31,35,64,0.04)] overflow-hidden border border-[#c9c4d5]/20">
        <div class="p-6 border-b border-[#c9c4d5]/10 flex justify-between items-center opacity-40 pointer-events-none select-none grayscale" title="Fitur filter sedang dalam tahap pengembangan">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-outline-variant/10">
                ...
                <tr class="hover:bg-[#fbf8ff] transition-colors cursor-pointer group" onclick="if(!event.target.closest('button') && !event.target.closest('a')) window.location='{{ route('admin.products.edit', $product->id) }}'">
                    ...
                    <td class="py-5 px-6">
                        @if($product->status == 1)
                            <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#b3f582]/20 border border-[#b3f582] text-on-tertiary-fixed-variant text-xs font-semibold">
